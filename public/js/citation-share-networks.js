(() => {
  ...
  function enhance(button) {
    if (!button || button.dataset.socialEnhanced === 'true') return;
    ...
    const description = publication.slice(0, 800);
    const socialSvg = `https://www.aupositeur.be/social/citations/${encodeURIComponent(slug)}.svg`;
    const mediaUrl = `https://images.weserv.nl/?url=${encodeURIComponent(socialSvg)}&output=png`;

    button.style.display = 'none';
    button.dataset.socialEnhanced = 'true';
  ...
  style.textContent = `
    ...
    @media (max-width: 700px) { .ap-unified-share__menu { right: auto; left: 0; } }
  `;
  document.head.appendChild(style);

  const enhanceAll = () => document.querySelectorAll('button[data-share-image]').forEach(enhance);
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', enhanceAll, { once: true });
  else enhanceAll();

  document.addEventListener('click', (event) => {
    if (event.target.closest?.('.ap-unified-share')) return;
    document.querySelectorAll('.ap-unified-share__menu').forEach((menu) => { menu.hidden = true; });
    document.querySelectorAll('.ap-unified-share > button').forEach((trigger) => trigger.setAttribute('aria-expanded', 'false'));
  });
})();
