...
async function loadProjects(){
  ...
  renderProjects();setText('[data-count="projects"]',String(state.projects.length).padStart(2,"0"));
}
function renderProjects(){
  ...
  list.querySelectorAll("[data-project]").forEach(b=>b.onclick=()=>{state.active=b.dataset.project;localStorage.setItem(ACTIVE_KEY,state.active);renderProjects()});
}
...
function renderAssets(){
  ...
  list.innerHTML=state.assets.length?state.assets.map(a=>`<div class="asset-row"><span>${esc(a.kind)}</span><b><a href="${API}/api/assets/${encodeURIComponent(a.id)}/content" target="_blank" rel="noopener">${esc(a.name)}</a></b><small>${esc(fmtBytes(a.bytes))}</small></div>`).join(""):'<div class="empty-state">Aucun asset dans R2 pour ce projet.</div>';
