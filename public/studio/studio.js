...
async function loadVisualReferences(){
  ...
  try{
    ...
    box.querySelectorAll("[data-ref-generate]").forEach(btn=>btn.onclick=async()=>{btn.disabled=true;btn.textContent="AGNES IMAGE CRÉE…";try{const vd=await api("/api/video/references/"+encodeURIComponent(btn.dataset.refGenerate)+"/variants"),existing=Math.min(4,(vd.variants||[]).length);const queueEl=document.querySelector("[data-agnes-queue-state]");if(queueEl)queueEl.innerHTML="<strong>File Agnes :</strong> IMAGE · EN COURS…";const result=await api("/api/video/references/"+encodeURIComponent(btn.dataset.refGenerate)+"/generate",{method:"POST",body:JSON.stringify({n:4,existing_count:existing,size:"1024x1024"})});await loadVisualReferences();await loadAgnesQueueState();if(!result.complete){alert("Génération partielle : "+result.total+" / "+result.requested+" variantes. Les images déjà créées sont conservées. Réessaie dans environ "+(result.retry_after||60)+" s pour générer uniquement les "+result.missing+" manquante(s).")}else if(existing>0){alert("Génération complétée : "+result.total+" / "+result.requested+" variantes. Seules les images manquantes ont été générées.")}}catch(e){alert("Génération image impossible : "+e.message);btn.disabled=false}});
