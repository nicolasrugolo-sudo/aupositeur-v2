...
async function loadVisualReferences(){
  ...
  try{
    ...
    box.querySelectorAll("[data-ref-generate]").forEach(btn=>btn.onclick=async()=>{btn.disabled=true;btn.textContent="AGNES IMAGE CRÉE…";try{const result=await api("/api/video/references/"+encodeURIComponent(btn.dataset.refGenerate)+"/generate",{method:"POST",body:JSON.stringify({n:4,size:"1024x1024"})});await loadVisualReferences();if(result.rate_limited){const n=result.variants?.length||0;alert("Agnes limite temporairement les requêtes. "+n+" variante(s) créée(s) sur 4. Réessaie dans environ "+(result.retry_after||60)+" s : les variantes déjà créées sont conservées.")}}catch(e){alert("Génération image impossible : "+e.message);btn.disabled=false}});
